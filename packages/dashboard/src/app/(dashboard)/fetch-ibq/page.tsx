...
export default function FetchIbqPage() {
  ...
  const fetchMutation = useMutation({
    ...
    onSuccess: (result) => {
      toast.success(
        `Fetched ${result.fetched} orders from IBQ — ingested: ${result.ingested}, skipped: ${result.skipped}`,
      );
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900">Fetch IBQ Orders</h1>
          <p className="mt-0.5 text-sm text-slate-500">
            Manually pull orders from the IBQ REST API and ingest them into the sync queue.
          </p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            >
              <option value="">
                {loadingCreds ? 'Loading credentials…' : '— Select a credential —'}
              </option>
          ...
          <Button
            ...
            disabled={fetchMutation.isPending || !credentialId}
            className="gap-2"
          >
