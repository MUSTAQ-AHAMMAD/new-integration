...
async function main() {
  ...
  if (queueEntries.length === 0) {
    ...
    console.log(
      '   RECOMMENDATION: Run POST /odoo-backup/fetch-orders or POST /ibq-backup/fetch-orders',
    );
    console.log();
    ...
    console.log(
      `✅ Found ${queueEntries.length} entry(ies) in OrderSyncQueue\n`,
    );

    for (const entry of queueEntries) {
      console.log('-'.repeat(80));
      console.log(
        `ENTRY: Order ${entry.odooOrderNumber} / Branch ${entry.branchCode}`,
      );
      console.log('-'.repeat(80));
      ...
      if (entry.validationErrors) {
        console.log(
          `Validation Errors: ${JSON.stringify(entry.validationErrors, null, 2)}`,
        );
      }
      ...
      if (entry.status === 'SKIPPED') {
        ...
        if (!entry.isPaid) {
          ...
          if (backup) {
            ...
            if (PAID_STATES.includes(state)) {
              ...
              console.log(
                '   🐛 BUG: Payment detection may have failed during ingestion',
              );
            } else {
              console.log(
                `   ❌ State "${backup.state}" is NOT in supported paid states`,
              );
              console.log(
                '   RECOMMENDATION: Check if this state should be considered paid',
              );
              console.log(
                '   If yes, it may need to be added to PAID_ORDER_STATES in odoo-utils.ts',
              );
            }
            ...
            if (rawData?.statement_ids) {
              console.log(
                `   Payment Data (statement_ids): ${JSON.stringify(rawData.statement_ids)}`,
              );
            }
            if (rawData?.payment_ids) {
              console.log(
                `   Payment Data (payment_ids): ${JSON.stringify(rawData.payment_ids)}`,
              );
            }
          }
        }
        ...
        if (entry.isCancelled) {
          ...
          console.log(
            '   RECOMMENDATION: Cancelled orders are intentionally skipped',
          );
        }
        ...
        if (!entry.isPaid && !entry.isCancelled) {
          console.log(
            '   1. Use the diagnose endpoint: GET /sync/orders/160909/diagnose',
          );
          console.log(
            '   2. If the order should be paid, use: POST /sync/orders/retry-skipped',
          );
          console.log(
            '   3. Or re-ingest from backup: POST /odoo-backup/reingest-from-backup',
          );
        }
      ...
      if (!storeConfig) {
        console.log(
          `❌ No store configuration found for branch ${entry.branchCode}`,
        );
      } else {
